...
AVPixelFormat video_buffer_pixel_format(const wl2::VideoBuffer& video) {
    if (auto format = video.format()) {
        switch (*format) {
            case wl2::VideoPixelFormat::Gray8: return AV_PIX_FMT_GRAY8;
            case wl2::VideoPixelFormat::Rgb24: return AV_PIX_FMT_RGB24;
            case wl2::VideoPixelFormat::Bgr24: return AV_PIX_FMT_BGR24;
            case wl2::VideoPixelFormat::Rgba32: return AV_PIX_FMT_RGBA;
            case wl2::VideoPixelFormat::Bgra32: return AV_PIX_FMT_BGRA;
            case wl2::VideoPixelFormat::Yuyv422: return AV_PIX_FMT_YUYV422;
            case wl2::VideoPixelFormat::Uyvy422: return AV_PIX_FMT_UYVY422;
        }
    }
    return video.bytesPerPixel() == 4 ? AV_PIX_FMT_RGBA : AV_PIX_FMT_RGB24;
}

int write_video_buffer_file(
    ...
    const std::string& preset,
    VideoBufferMuxResult& result) {
    auto opened = wl2::VideoBuffer::openExisting(bufferName);
    if (!opened) {
        return AVERROR(EINVAL);
    }
    wl2::VideoBuffer video = std::move(opened.value());
    ...
    const AVCodec* codec = find_encoder_preference(video_encoder_candidates(videoCodec));
    if (!codec) {
        return AVERROR_ENCODER_NOT_FOUND;
    }
    ...
    enc->width = width;
    enc->height = height;
    enc->pix_fmt = encoder_first_pixel_format(codec, AV_PIX_FMT_YUV420P);
    enc->time_base = AVRational{1, fps};
    enc->framerate = AVRational{fps, 1};
    apply_video_preset(enc, preset);
    if (outFmt->oformat->flags & AVFMT_GLOBALHEADER) {
    ...
    AVStream* videoStream = avformat_new_stream(outFmt, nullptr);
    if (!videoStream) {
        return AVERROR(ENOMEM);
    }
    videoStream->time_base = enc->time_base;
    videoStream->avg_frame_rate = AVRational{fps, 1};
    videoStream->r_frame_rate = AVRational{fps, 1};
    ret = avcodec_parameters_from_context(videoStream->codecpar, enc);
    if (ret < 0) {
        return ret;
    }
    ...
    const AVPixelFormat srcFormat = video_buffer_pixel_format(video);
    SwsContext* sws = sws_getContext(width, height, srcFormat, width, height, enc->pix_fmt,
        SWS_BILINEAR, nullptr, nullptr, nullptr);
    if (!sws) {
        return AVERROR(EINVAL);
    }
    auto freeSws = [](SwsContext* s) { if (s) sws_freeContext(s); };
    std::unique_ptr<SwsContext, decltype(freeSws)> swsGuard(sws, freeSws);
    ...
    auto writePackets = [&]() -> int {
        int writeRet = 0;
        while ((writeRet = avcodec_receive_packet(enc, packet)) >= 0) {
            packet->stream_index = videoStream->index;
            av_packet_rescale_ts(packet, enc->time_base, videoStream->time_base);
            writeRet = av_interleaved_write_frame(outFmt, packet);
            av_packet_unref(packet);
            if (writeRet < 0) {
                return writeRet;
            }
        ...
        return writeRet == AVERROR(EAGAIN) || writeRet == AVERROR_EOF ? 0 : writeRet;
    };

    for (int i = 0; i < frameCount; ++i) {
        auto view = video.frame(startSlot + i);
        if (!view) {
            ret = AVERROR(EINVAL);
            break;
        }
        ...
        const uint8_t* srcData[4] = {reinterpret_cast<const uint8_t*>(view.value().data), nullptr, nullptr, nullptr};
        int srcLinesize[4] = {static_cast<int>(view.value().scanWidth), 0, 0, 0};
    ...
    result.container = outFmt->oformat->name ? outFmt->oformat->name : "";
    result.videoCodec = codec->name ? codec->name : "";
    result.videoPixelFormat = pixel_format_name(enc->pix_fmt);
    result.width = width;
    ...
    AVFormatContext* verify = nullptr;
    if (avformat_open_input(&verify, outputPath.c_str(), nullptr, nullptr) >= 0) {
        result.reopened = avformat_find_stream_info(verify, nullptr) >= 0;
        if (result.reopened) {
            result.reopenedStreams = verify->nb_streams;
            for (unsigned i = 0; i < verify->nb_streams; ++i) {
                AVCodecParameters* par = verify->streams[i]->codecpar;
                if (par->codec_type == AVMEDIA_TYPE_VIDEO) {
                    result.reopenedWidth = par->width;
                    result.reopenedHeight = par->height;
                    break;
...
wl2::PacketKind packet_kind_for_stream(AVFormatContext* format, int streamIndex) {
    if (streamIndex >= 0 && streamIndex < static_cast<int>(format->nb_streams)) {
        const AVMediaType type = format->streams[streamIndex]->codecpar->codec_type;
...
std::string make_packet_record_header(const AVPacket* packet, uint64_t sequence, bool discontinuity) {
    std::string record = make_sequenced_packet_record(packet, sequence, discontinuity);
    record.resize(60);
    return record;
}

int stream_through_packet_buffer(
    const std::string& inPath,
    const std::string& outPath,
    const std::string& bufferName,
    int64_t buffers,
    ...
    const std::string streamMetadata = make_stream_config_record(inFmt);
    auto created = wl2::PacketBuffer::create(bufferName, buffers, arenaSize, maxRecord, 0, 0, streamMetadata);
    if (!created) {
        return AVERROR_EXTERNAL;
    }
    auto reader = wl2::PacketBuffer::openExisting(bufferName);
    if (!reader) {
        return AVERROR_EXTERNAL;
    }
    wl2::PacketBuffer& writer = created.value();
    wl2::PacketBuffer& packetReader = reader.value();
    if (packetReader.metadata().compare(0, 7, "WL2FSC1") != 0) {
    ...
    uint64_t sequence = 0;
    int inFlight = 0;
    std::vector<int64_t> pendingSlots;
    ...
    auto consume_one = [&]() -> int {
        ...
        const int64_t slot = pendingSlots.front();
        pendingSlots.erase(pendingSlots.begin());
        auto record = packetReader.record(slot);
        if (!record) {
            return -1;
        }
        ++result.recordsRead;
        --inFlight;
        const std::string& meta = record.value().metadata;
        const std::string& payload = record.value().payload;
        constexpr size_t kHeaderSize = 60;
        if (meta.size() < kHeaderSize || meta.compare(0, 7, "WL2FQP2") != 0) {
            return -1;
        }
        const int streamIndex = static_cast<int>(read_u32(meta.data() + 12));
        const int64_t pts = read_i64(meta.data() + 16);
        const int64_t dts = read_i64(meta.data() + 24);
        const int64_t duration = read_i64(meta.data() + 32);
        const uint32_t flags = read_u32(meta.data() + 40);
        const uint32_t size = read_u32(meta.data() + 44);
        const uint32_t discontinuity = read_u32(meta.data() + 56);
        if (discontinuity) {
        ...
        if (!comparedFirstPayload && !firstSourcePayload.empty()) {
            comparedFirstPayload = true;
            result.payloadPreserved = payload.size() == firstSourcePayload.size()
                && std::memcmp(payload.data(), firstSourcePayload.data(), firstSourcePayload.size()) == 0;
        }
        ...
        std::memcpy(outPacket->data, payload.data(), size);
        ...
        AVStream* outStream = outFmt->streams[mapped];
        av_packet_rescale_ts(outPacket, sourceTimeBase[streamIndex], outStream->time_base);
        outPacket->pos = -1;
    ...
    while ((ret = av_read_frame(inFmt, packet)) >= 0) {
        ...
        const std::string meta = make_packet_record_header(packet, sequence++, discontinuity);
        if (firstSourcePayload.empty() && packet->size > 0) {
            firstSourcePayload.assign(reinterpret_cast<const char*>(packet->data), static_cast<size_t>(packet->size));
        }
        auto written = writer.write(std::string_view(reinterpret_cast<const char*>(packet->data), static_cast<size_t>(packet->size)),
            packet_kind_for_stream(inFmt, packet->stream_index), packet->stream_index, packet->pts, meta);
        if (!written) {
            av_packet_unref(packet);
            break;
        }
        const int64_t slot = writer.buffers() > 0 ? (written.value() + writer.buffers() - 1) % writer.buffers() : -1;
...
JSValue muxer_write_video_buffer(JSContext* ctx, JSValueConst, int argc, JSValueConst* argv) {
    if (argc < 1 || !JS_IsObject(argv[0])) {
        return JS_ThrowTypeError(ctx, "Muxer.writeVideoBuffer(options) requires an options object");
    }
    ...
    std::string outputPath;
    if (!get_string_prop(ctx, argv[0], "outputPath", outputPath)) {
        get_string_prop(ctx, argv[0], "path", outputPath);
    }
    int32_t fps = 30;
    int32_t frames = 0;
    int32_t startSlot = 0;
    get_int_prop(ctx, argv[0], "fps", fps);
    get_int_prop(ctx, argv[0], "frames", frames);
    get_int_prop(ctx, argv[0], "startSlot", startSlot);
    std::string videoCodec;
    std::string preset = "low-latency";
    get_string_prop(ctx, argv[0], "videoCodec", videoCodec);
    get_string_prop(ctx, argv[0], "preset", preset);
    if (bufferName.empty() || outputPath.empty() || frames <= 0) {
        return JS_ThrowTypeError(ctx, "Muxer.writeVideoBuffer requires videoBufferName, outputPath, and frames");
    }

    VideoBufferMuxResult result;
    int ret = write_video_buffer_file(bufferName, outputPath, fps, frames, startSlot, videoCodec, preset, result);
    if (ret < 0) {
        return throw_ffmpeg_error(ctx, "Muxer.writeVideoBuffer", ret);
    }
    ...
    JS_SetPropertyStr(ctx, obj, "width", JS_NewInt32(ctx, result.width));
    JS_SetPropertyStr(ctx, obj, "height", JS_NewInt32(ctx, result.height));
    JS_SetPropertyStr(ctx, obj, "fps", JS_NewInt32(ctx, result.fps));
    JS_SetPropertyStr(ctx, obj, "frames", JS_NewInt64(ctx, result.frames));
    JS_SetPropertyStr(ctx, obj, "videoPackets", JS_NewInt64(ctx, result.videoPackets));
    set_bool(ctx, obj, "reopened", result.reopened);
    JS_SetPropertyStr(ctx, obj, "reopenedStreams", JS_NewInt32(ctx, result.reopenedStreams));
    JS_SetPropertyStr(ctx, obj, "reopenedWidth", JS_NewInt32(ctx, result.reopenedWidth));
...
JSValue packet_queue_stream_through_packet_buffer(JSContext* ctx, JSValueConst, int argc, JSValueConst* argv) {
    if (argc < 1 || !JS_IsObject(argv[0])) {
        return JS_ThrowTypeError(ctx, "PacketQueue.streamThroughPacketBuffer(options) requires an options object");
    }
    std::string inputPath;
    std::string outputPath;
    std::string bufferName;
    std::string policy = "lossless";
    int32_t buffers = 64;
    int32_t arenaSize = 1 << 20;
    int32_t maxRecord = 1 << 20;
    int32_t maxDepth = 8;
    get_string_prop(ctx, argv[0], "path", inputPath);
    if (inputPath.empty()) {
        get_string_prop(ctx, argv[0], "inputPath", inputPath);
    ...
    get_string_prop(ctx, argv[0], "outputPath", outputPath);
    if (!get_string_prop(ctx, argv[0], "packetBufferName", bufferName)) {
        get_string_prop(ctx, argv[0], "bufferName", bufferName);
    }
    get_string_prop(ctx, argv[0], "policy", policy);
    get_int_prop(ctx, argv[0], "buffers", buffers);
    get_int_prop(ctx, argv[0], "arenaSize", arenaSize);
    get_int_prop(ctx, argv[0], "maxRecord", maxRecord);
    get_int_prop(ctx, argv[0], "maxDepth", maxDepth);
    if (inputPath.empty() || outputPath.empty() || bufferName.empty()) {
        return JS_ThrowTypeError(ctx, "PacketQueue.streamThroughPacketBuffer requires path, outputPath, and packetBufferName");
    }
    if (policy != "lossless" && policy != "dropOldestUntilKeyframe") {
        return throw_module_error(ctx, "PacketQueue.streamThroughPacketBuffer", "policy_invalid",
            "policy must be lossless or dropOldestUntilKeyframe");
    }
    buffers = std::clamp<int32_t>(buffers, 2, 65536);
    maxDepth = std::clamp<int32_t>(maxDepth, 1, buffers);
    maxRecord = std::max<int32_t>(maxRecord, 4096);
    arenaSize = std::max<int32_t>(arenaSize, maxRecord * 2);

    QueueRemuxResult result;
    int ret = stream_through_packet_buffer(inputPath, outputPath, bufferName, buffers, arenaSize, maxRecord,
        policy, maxDepth, result);
    if (ret < 0) {
        return throw_ffmpeg_error(ctx, "PacketQueue.streamThroughPacketBuffer", ret);
    }
    ...
    set_string(ctx, obj, "container", result.remux.container);
    set_string(ctx, obj, "outputPath", outputPath);
    set_string(ctx, obj, "packetBufferName", bufferName);
    JS_SetPropertyStr(ctx, obj, "packetsRead", JS_NewInt64(ctx, result.packetsRead));
    JS_SetPropertyStr(ctx, obj, "recordsWritten", JS_NewInt64(ctx, result.recordsWritten));
    JS_SetPropertyStr(ctx, obj, "recordsRead", JS_NewInt64(ctx, result.recordsRead));
    JS_SetPropertyStr(ctx, obj, "dropped", JS_NewInt64(ctx, result.dropped));
...
JSValue make_muxer_namespace(JSContext* ctx) {
    ...
    JS_SetPropertyStr(ctx, ns, "writeVideoBuffer", JS_NewCFunction(ctx, muxer_write_video_buffer, "writeVideoBuffer", 1));
    return ns;
}
...
JSValue make_packet_queue_namespace(JSContext* ctx) {
    ...
    JS_SetPropertyStr(ctx, ns, "streamThroughPacketBuffer",
        JS_NewCFunction(ctx, packet_queue_stream_through_packet_buffer, "streamThroughPacketBuffer", 1));
    return ns;
}
