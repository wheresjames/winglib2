...
int main() {
    ...
    const auto videoBufferMuxPath = std::filesystem::temp_directory_path()
        / ("wl2-ffmpeg-videobuffer-" + std::to_string(getpid()) + ".avi");
    const auto queueRemuxPath = std::filesystem::temp_directory_path()
        / ("wl2-ffmpeg-queue-" + std::to_string(getpid()) + ".avi");
    const auto packetBufferRemuxPath = std::filesystem::temp_directory_path()
        / ("wl2-ffmpeg-packet-buffer-" + std::to_string(getpid()) + ".avi");
    ...
    const std::string packetBufferName = "/wl2_ffmpeg_pktbuf_" + std::to_string(getpid());
...
const videoBufferMuxPath = ")JS" << videoBufferMuxPath.string() << R"JS(";
const queueRemuxPath = ")JS" << queueRemuxPath.string() << R"JS(";
const packetBufferRemuxPath = ")JS" << packetBufferRemuxPath.string() << R"JS(";
...
const packetBufferName = ")JS" << packetBufferName << R"JS(";
...
const videoBufferMuxed = Muxer.writeVideoBuffer({
  videoBufferName,
  outputPath: videoBufferMuxPath,
  fps: 30,
  frames: 1,
  startSlot: published.slot
});
assert(videoBufferMuxed.frames === 1, "VideoBuffer mux frame count mismatch");
assert(videoBufferMuxed.videoPackets > 0, "VideoBuffer mux produced no packets");
assert(videoBufferMuxed.reopened === true, "VideoBuffer mux output should reopen");
assert(videoBufferMuxed.reopenedWidth === 160 && videoBufferMuxed.reopenedHeight === 90, "VideoBuffer mux dimensions mismatch");
...
assert(caps.videoBufferMux === true, "VideoBuffer mux capability missing");
...
// PacketQueue.streamThroughPacketBuffer: compressed packets through libmembus mempkt.
if (caps.packetBufferStream) {
  const packetBuffered = PacketQueue.streamThroughPacketBuffer({
    path: fixturePath,
    outputPath: packetBufferRemuxPath,
    packetBufferName,
    policy: "lossless",
    buffers: 64,
    arenaSize: 1048576,
  ...
  assert(packetBuffered.policy === "lossless", "packet buffer policy mismatch");
  assert(packetBuffered.packetsRead > 0, "packet buffer stream read no packets");
  assert(packetBuffered.recordsWritten === packetBuffered.packetsRead, "packet buffer stream should not drop records");
  assert(packetBuffered.dropped === 0, "packet buffer lossless policy must not drop");
  assert(packetBuffered.payloadPreserved === true, "packet buffer payload not preserved");
  assert(packetBuffered.timestampsPreserved === true, "packet buffer timestamps not preserved");
  assert(packetBuffered.extradataPreserved === true, "packet buffer extradata not preserved");
}
...
assert(profile.perRecordUs >= 0, "packet queue profiling should report per-record cost");
    ...
    std::filesystem::remove(videoBufferMuxPath, cleanupError);
    std::filesystem::remove(queueRemuxPath, cleanupError);
    std::filesystem::remove(packetBufferRemuxPath, cleanupError);
